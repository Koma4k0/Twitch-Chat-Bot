...
module.exports = {
    name: 'delmod',
    use: '<user>',
    description: '[BOT OWNER ONLY] Mod command to remove a user as a moderator',
    async execute(client, channel, tags, message, args) {
        const isOwner = await this.checkIfOwner(tags);

        if (!isOwner) {
            client.say(channel, 'Sorry, only the owner of the bot can use this command.');
            return;
        }
        ...
        if (!mentionedUsername) {
            client.say(channel, 'Please mention the user you want to remove as a moderator.');
            return;
        }
        ...
        if (!isModExist) {
            client.say(channel, `${cleanMentionedUsername} is not a moderator.`);
            return;
        }

        modDB.run('DELETE FROM mods WHERE user_id=?', [mentionedUserId]);

        await client.say(channel, `Successfully removed ${cleanMentionedUsername} as a moderator.`);
    },
